...
func (db *DatabaseBigQuery) Init(ctx context.Context, project string, dataset string,
	...
	slog.Info("Opening connection to BigQuery.")
...
func (db *DatabaseBigQuery) SendOrder(order Order) error {
	...
	for _, sql := range queries {
		...
		if err != nil {
			slog.Error("Error submitting query.")
			return err
		}

		status, err := job.Wait(db.ctx)
		if err != nil {
			slog.Error("Error waiting for query to finish.")
			return err
		}
		if status.Err() != nil {
			return status.Err()
...
func (db *DatabaseBigQuery) SendOrders(orders []Order) error {
	return nil
}
...
func (db *DatabaseBigQuery) GetOpenOrders() (Orders, error) {
	...
	for {
		...
		if it.Next(&order) == iterator.Done {
			break
		}
...
func (dummy *DatabaseBigQueryDummy) Init(ctx context.Context, project string, dataset string,
	orders_table string, order_items_table string, timezone string) error {
	slog.Info("Opening DummyDatabase-connection.")
...
func (dummy *DatabaseBigQueryDummy) Close() {
	slog.Info("Closing DummyDatabase-connection.")
}
...
func (dummy *DatabaseBigQueryDummy) SendOrders(orders []Order) error {
	return nil
}

func (dummy *DatabaseBigQueryDummy) GetOpenOrders() (Orders, error) {
	slog.Info("Getting open orders from DummyDatabase.")
